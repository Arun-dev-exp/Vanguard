  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => setShowAddForm(!showAddForm)} className="flex items-center gap-2 bg-primary text-on-primary px-4 py-2 rounded-[0.25rem] font-bold text-xs hover:bg-primary-fixed transition-colors">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1">
        ...
        <div className="lg:col-span-8 glass-panel glass-panel-hover rounded-lg flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {rules.map(rule => {
              const sevMap = { critical: 'text-error', high: 'text-[#FF9F0A]', medium: 'text-[#FFD60A]' };
              const sevBg = { critical: 'bg-error/10 border-error', high: 'bg-[#FF9F0A]/10 border-[#FF9F0A]', medium: 'bg-[#FFD60A]/10 border-[#FFD60A]' };
              return (
                <div key={rule.id} className={`p-4 rounded-lg border transition-all ${rule.status === 'active' ? 'border-white/10 bg-surface-container-low' : 'border-white/5 bg-surface-container-lowest/50 opacity-60'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <button onClick={() => toggleRule(rule.id)} className={`w-10 h-5 rounded-full relative transition-colors ${rule.status === 'active' ? 'bg-tertiary-container' : 'bg-outline-variant'}`}>
                        <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${rule.status === 'active' ? 'left-5' : 'left-0.5'}`}></span>
                      </button>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => toggleAutoAction(rule.id)} className={`w-8 h-4 rounded-full relative transition-colors ${rule.autoAction ? 'bg-primary' : 'bg-outline-variant'}`}>
                        <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${rule.autoAction ? 'left-4' : 'left-0.5'}`}></span>
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          ...
          <div className="glass-panel glass-panel-hover rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-data-display-md text-[#FF9F0A]">{rules.filter(r => r.status === 'paused').length}</span>
              </div>
              <div className="flex justify-between items-center">
          ...
          {/* Add Rule Form */}
          {showAddForm && (
            <div className="glass-panel rounded-lg p-6 border border-primary/30 animate-fade-in">
              <h3 className="text-label-caps text-on-surface mb-4">NEW RULE</h3>
              <div className="space-y-3">
                <input className="w-full bg-[#05070A] border border-white/10 rounded-[0.25rem] px-3 py-2 text-body-main text-on-surface focus:outline-none focus:border-primary transition-all text-xs" placeholder="Rule name" />
                <input className="w-full bg-[#05070A] border border-white/10 rounded-[0.25rem] px-3 py-2 text-body-main text-on-surface focus:outline-none focus:border-primary transition-all text-xs" placeholder="Description" />
                <select className="w-full bg-[#05070A] border border-white/10 rounded-[0.25rem] px-3 py-2 text-body-main text-on-surface focus:outline-none focus:border-primary transition-all text-xs">
                  <option>Critical</option><option>High</option><option>Medium</option>
                </select>
                <input className="w-full bg-[#05070A] border border-white/10 rounded-[0.25rem] px-3 py-2 text-body-main text-on-surface focus:outline-none focus:border-primary transition-all text-xs" placeholder="Threshold" type="number" />
                <button className="w-full bg-primary text-on-primary py-2 rounded-[0.25rem] font-bold text-xs hover:bg-primary-fixed transition-colors">CREATE RULE</button>
              </div>
            </div>
          )}
        </div>
